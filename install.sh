...
# @@File             :  install.sh
# @@Description      :  Container installer script for supabase
...
REPO="${DOCKERMGRREPO:-https://github.com/dockermgr}/supabase"
APPVERSION="$(__appversion "$REPO/raw/${GIT_REPO_BRANCH:-main}/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="supabase"
export APPDIR="$HOME/.local/share/srv/docker/supabase"
export DATADIR="$HOME/.local/share/srv/docker/supabase/rootfs"
export INSTDIR="$HOME/.local/share/CasjaysDev/dockermgr/supabase"
...
HUB_IMAGE_URL="casjaysdevdocker/supabase"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# image tag [docker pull HUB_IMAGE_URL:tag]
HUB_IMAGE_TAG="latest"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the container name Default: [casjaysdevdocker/supabase-$HUB_IMAGE_TAG]
CONTAINER_NAME=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: supabase
CONTAINER_HOSTNAME=""
...
if [ -z "$CONTAINER_NAME" ]; then
  CONTAINER_NAME="$(__container_name || echo "casjaysdevdocker/supabase-$HUB_IMAGE_TAG")"
fi
...
if [ -n "$SUPABASE_USERNAME" ]; then
  CONTAINER_USER_NAME="$SUPABASE_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${SUPABASE_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
